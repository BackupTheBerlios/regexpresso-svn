	...
	*/
	smoothShow: function(options){
		...
		if(this.getStyle('display') == "none" || 
			...
			if(this.fxOpacityOk()) startStyles.opacity = 1;
			var zero = { height: '0px' };
			$each(startStyles, function(style, name){ zero[name] = (name.test('width')||name.test('height'))?'0px':0; });
...
if(typeof Mootools == "undefined" || !(Mootools && Mootools.version && Mootools.version.toFloat()>1)) {
	Element.extend({
		/*
		Property: empty
